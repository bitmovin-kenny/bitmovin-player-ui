 ...
 */
export interface AudioSettingsToggleButtonConfig extends ToggleButtonConfig {
  /**
   * The settings panel whose visibility the button should toggle.
   */
  settingsPanel: AudioSettingsPanel;
...
/**
 * A button that toggles visibility of a audio settings panel.
 */
export class AudioSettingsToggleButton extends ToggleButton<AudioSettingsToggleButtonConfig> {

  private visibleSettingsPanels: AudioSettingsPanel[] = [];

  constructor(config: AudioSettingsToggleButtonConfig) {
    super(config);
    ...
    this.config = this.mergeConfig(config, {
      cssClass: 'ui-audiosettingstogglebutton',
      text: 'AudioSettings',
      settingsPanel: null,
      autoHideWhenNoActiveSettings: true,
    }, <AudioSettingsToggleButtonConfig>this.config);
  }

  configure(player: PlayerAPI, uimanager: UIInstanceManager): void {
    super.configure(player, uimanager);

    let config = <AudioSettingsToggleButtonConfig>this.getConfig(); // TODO fix generics type inference
